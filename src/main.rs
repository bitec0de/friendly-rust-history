...
<html lang="en-US">
<head>
<title>Rust Release History</title>
<meta charset="utf-8">
<style>
    h2,h3 {margin: 0;}
...
<body>
<p>Original at: <a href="https://github.com/rust-lang/rust/blob/master/RELEASES.md">https://github.com/rust-lang/rust/blob/master/RELEASES.md</a><!-- , this page generated by <a href="https://github.com/bitec0de/friendly-rust-history/">this bad program</a>. -->
"#.to_owned();
            ...
            match para {
                mdast::Node::Heading(heading) =>
                    if start_of_section {
                        ...
                        acc.push_str(&std::format!("\n<div><span class='sticky-subheader solid-background' style='display: inline-block;'><h3>{}</h3></span>\n", child_nodes_to_html(&heading.children, &definitions)));
    ...
    for n in nodes {
        match n {
            ...
            mdast::Node::Paragraph(p) =>
                acc.push_str(&format!("<p>{}</p>", child_nodes_to_html(&p.children, &definitions))),
            ...
            mdast::Node::Delete(del) =>
                acc.push_str(&format!("<span style='text-decoration: line-through;'>{}</span>", child_nodes_to_html(&del.children, &definitions))),
            mdast::Node::Emphasis(em) =>
                acc.push_str(&format!("<em>{}</em>", child_nodes_to_html(&em.children, &definitions))),
            mdast::Node::Strong(strong) =>
                acc.push_str(&format!("<b><i>{}</i></b>", child_nodes_to_html(&strong.children, &definitions))),
            mdast::Node::InlineCode(inline) =>
                acc.push_str(&format!("<span class='code'>{}</span>", &inline.value)),
            mdast::Node::Code(code) =>
